...
████████████████████████████████████████████████████████████████████████████████████
██                                                                                ██
██  ⚙️  ABYSSAL ENGINE v2.0 — Real AI Execution × Leviathan Arsenal  ⚙️         ██
██                                                                                ██
██  The convergence point. Where Shannon's brain meets Leviathan's body.          ██
...
██    5. Falls back to Leviathan-native tool execution when no AI                 ██
...
logger = logging.getLogger("leviathan.shannon.engine")
...
class EngineConfig:
    ...
    Combines Shannon parameters (target, repo, auth) with Leviathan
    ...
    leviathan_domains: Optional[List[str]] = None
...
- Tools used (Leviathan VS arsenal)
...
# ════════════════════════════════════════════════════════════════════════════
# LEVIATHAN TOOL DEFINITIONS — For AI function calling
# ════════════════════════════════════════════════════════════════════════════

LEVIATHAN_TOOLS: List[Dict[str, Any]] = [
    {
        "name": "scan_ports",
...
class AbyssalEngine:
    """The autonomous engine that fuses Shannon + Leviathan.
    ...
    async def preflight(self) -> Dict[str, Any]:
        ...
        # 5. Leviathan tools
        lev = self._check_leviathan_availability()
        report["checks"].append(
            {
                "name": "Leviathan Arsenal",
    ...
    def _check_leviathan_availability(self) -> Dict[str, Any]:
        """Check which Leviathan domain modules are available."""
        domain_modules = {
    ...
    def _build_prompt(self, agent: AgentDefinition) -> str:
        """Build complete prompt for an agent with full Leviathan context."""
        ...
        # Leviathan context
        lev_lines = [
            "## LEVIATHAN VS ARSENAL",
            f"  Active Domains: {', '.join(agent.leviathan_domains)}",
        ...
        if agent.name == "report":
            ...
            context["tools_used"] = json.dumps(
                {d: True for d in agent.leviathan_domains},
                indent=2,
            )
    ...
    async def _execute_via_ai(
        ...
        system_prompt = (
            ...
            f"in the SIREN pipeline (Shannon × Leviathan VS). "
            f"Phase: {agent.phase}. "
        ...
        tools = LEVIATHAN_TOOLS if self.config.enable_tool_use else None
    ...
    async def _execute_locally(
        ...
        """Execute agent locally using Leviathan tools (no AI).
        ...
        parts = [
            ...
            f"**Mode**: Leviathan-native (no AI provider)",
            "",
        ]
    ...
    async def _local_code_analysis(self, agent: AgentDefinition) -> List[str]:
        """Local code analysis using Leviathan scanner."""
        ...
        lines.append("\n---\n*Generated by Leviathan VS local scanner*\n")
        return lines
    ...
    async def _local_recon(self, agent: AgentDefinition) -> List[str]:
        ...
        try:
            ...
            try:
                ...
                req.add_header("User-Agent", "SIREN/2.0 (Leviathan VS)")
        ...
        lines.append("---\n*Generated by Leviathan VS local recon*\n")
        return lines
